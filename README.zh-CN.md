...
```bash
# 走 Homebrew（需先发布 v3.0.0 并建好 loeweErr/tap）：
...
脚本会自动检测工具链（完整 Xcode 还是 CommandLineTools-only）、构建 `ClaudePet.app`、安装到 `/Applications/ClaudePet.app`（可在交互中改），并把 `claude-pet` MCP server 注册到 `~/Library/Application Support/Claude/claude_desktop_config.json`，**不会覆盖**你已有的其它 MCP server，且会在原文件旁留一个带时间戳的 `.bak` 备份。

完成后 **Cmd+Q 完全退出 Claude Desktop 并重启**，在对话里说"喂猫"试试。
